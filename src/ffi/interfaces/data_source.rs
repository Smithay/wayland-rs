...
pub unsafe fn wl_data_source_set_user_data(data_source: *mut wl_data_source, data: *mut c_void) {
    abi::wl_proxy_set_user_data(data_source as *mut wl_proxy, data)
}

#[inline(always)]
pub unsafe fn wl_data_source_get_user_data(data_source: *mut wl_data_source) -> *mut c_void {
    abi::wl_proxy_get_user_data(data_source as *mut wl_proxy)
}
...
pub unsafe fn wl_data_source_destroy(data_source: *mut wl_data_source) {
    abi::wl_proxy_marshal(data_source as *mut wl_proxy, WL_DATA_SOURCE_DESTROY);
    abi::wl_proxy_destroy(data_source as *mut wl_proxy)
}