...
pub unsafe fn wl_shm_pool_set_user_data(shm_pool: *mut wl_shm_pool, data: *mut c_void) {
    abi::wl_proxy_set_user_data(shm_pool as *mut wl_proxy, data)
}

#[inline(always)]
pub unsafe fn wl_shm_pool_get_user_data(shm_pool: *mut wl_shm_pool) -> *mut c_void {
    abi::wl_proxy_get_user_data(shm_pool as *mut wl_proxy)
}
...
pub unsafe fn wl_shm_pool_destroy(shm_pool: *mut wl_shm_pool) {
    abi::wl_proxy_marshal(shm_pool as *mut wl_proxy, WL_SHM_POOL_DESTROY);
    abi::wl_proxy_destroy(shm_pool as *mut wl_proxy)
}

#[inline(always)]
pub unsafe fn wl_shm_pool_resize(shm_pool: *mut wl_shm_pool, size: int32_t) {
    abi::wl_proxy_marshal(shm_pool as *mut wl_proxy, WL_SHM_POOL_RESIZE, size)
}