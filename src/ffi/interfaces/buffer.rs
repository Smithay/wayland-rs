...
pub unsafe fn wl_buffer_set_user_data(buffer: *mut wl_buffer, data: *mut c_void) {
    abi::wl_proxy_set_user_data(buffer as *mut wl_proxy, data)
}

#[inline(always)]
pub unsafe fn wl_buffer_get_user_data(buffer: *mut wl_buffer) -> *mut c_void {
    abi::wl_proxy_get_user_data(buffer as *mut wl_proxy)
}

#[inline(always)]
pub unsafe fn wl_buffer_destroy(buffer: *mut wl_buffer) {
    abi::wl_proxy_marshal(buffer as *mut wl_proxy, WL_BUFFER_DESTROY);
    abi::wl_proxy_destroy(buffer as *mut wl_proxy)
}